import React from "react";
import './match.css';

const Match = () => {
    return (
        <section className="match section" id="match">
            <h2 className="section__title">Matches</h2>
            <span className="section__subtitle">Game schedule</span>

            <div className="match__container container grid">
                <div className="match__content 1">
                    <h3 className="match__title">Match Day 1</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    07 Jan, 2023 to 08 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓌ vs KafitiⓁ</li>
                                        <li>arnie_NⓌ vs KazremyⓁ</li>
                                        <li>raheemthefirstⓌ vs BriodeeⓁ</li>
                                        <li>ClusteredFib3rⓌ vs k_dash34Ⓛ</li>
                                        <li>alphazzz3r0Ⓛ vs dinokapon26Ⓦ</li>
                                        <li>Andre_BeatzⓁ vs somrandomguyⓌ</li>
                                        <li>FarajahchalloⓁ vs HopertzⓌ</li>
                                        <li>MABESSAⓁ vs GN02Ⓦ</li>
                                        <li>musir6363Ⓛ vs BiGFiVeWin05Ⓦ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 2">
                    <h3 className="match__title">Match Day 2</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    07 Jan, 2023 to 08 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>KazremyⓁ vs onesmoⓌ</li>
                                        <li>BriodeeⓁ vs KafitiⓌ</li>
                                        <li>k_dash34Ⓦ vs arnie_NⓁ</li>
                                        <li>dinokapon26Ⓦ vs raheemthefirstⓁ</li>
                                        <li>somrandomguyⓁ vs ClusteredFib3rⓌ</li>
                                        <li>HopertzⓌ vs alphazzz3r0Ⓛ</li>
                                        <li>GN02Ⓦ vs Andre_BeatzⓁ</li>
                                        <li>BiGFiVeWin05Ⓦ vs FarajahchalloⓁ</li>
                                        <li>musir6363Ⓛ vs MABESSAⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 3">
                    <h3 className="match__title">Match Day 3</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    07 Jan, 2023 to 08 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓌ vs BriodeeⓁ</li>
                                        <li>KazremyⓁ vs k_dash34Ⓦ</li>
                                        <li>KafitiⓁ vs dinokapon26Ⓦ</li>
                                        <li>arnie_NⓁ vs somrandomguyⓌ</li>
                                        <li>raheemthefirstⓁ vs HopertzⓌ</li>
                                        <li>ClusteredFib3rⓁ vs GN02Ⓦ</li>
                                        <li>alphazzz3r0Ⓛ vs BiGFiVeWin05Ⓦ</li>
                                        <li>Andre_BeatzⓁ vs musir6363Ⓦ</li>
                                        <li>FarajahchalloⓁ vs MABESSAⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 4">
                    <h3 className="match__title">Match Day 4</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    14 Jan, 2023 to 15 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>k_dash34Ⓛ vs onesmoⓌ</li>
                                        <li>dinokapon26Ⓦ vs BriodeeⓁ</li>
                                        <li>somrandomguyⓁ vs KazremyⓌ</li>
                                        <li>HopertzⓁ vs KafitiⓌ</li>
                                        <li>GN02Ⓦ vs arnie_NⓁ</li>
                                        <li>BiGFiVeWin05Ⓦ vs raheemthefirstⓁ</li>
                                        <li>musir6363Ⓛ vs ClusteredFib3rⓌ</li>
                                        <li>MABESSAⓌ vs alphazzz3r0Ⓛ</li>
                                        <li>FarajahchalloⓁ vs Andre_BeatzⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 5">
                    <h3 className="match__title">Match Day 5</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    14 Jan, 2023 to 15 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓁ vs dinokapon26Ⓦ</li>
                                        <li>k_dash34Ⓛ vs somrandomguyⓌ</li>
                                        <li>BriodeeⓁ vs HopertzⓌ</li>
                                        <li>KazremyⓁ vs GN02Ⓦ</li>
                                        <li>KafitiⓌ vs BiGFiVeWin05Ⓛ</li>
                                        <li>arnie_NⓌ vs musir6363Ⓛ</li>
                                        <li>raheemthefirstⓁ vs MABESSAⓌ</li>
                                        <li>ClusteredFib3rⓌ vs FarajahchalloⓁ</li>
                                        <li>alphazzz3r0Ⓛ vs Andre_BeatzⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 6">
                    <h3 className="match__title">Match Day 6</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    14 Jan, 2023 to 15 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>somrandomguyⓁ vs onesmoⓌ</li>
                                        <li>HopertzⓁ vs dinokapon26Ⓦ</li>
                                        <li>GN02Ⓦ vs k_dash34Ⓛ</li>
                                        <li>BiGFiVeWin05Ⓦ vs BriodeeⓁ</li>
                                        <li>musir6363Ⓓ vs KazremyⒹ</li>
                                        <li>MABESSAⓁ vs KafitiⓌ</li>
                                        <li>FarajahchalloⓁ vs arnie_NⓌ</li>
                                        <li>Andre_BeatzⓁ vs raheemthefirstⓌ</li>
                                        <li>alphazzz3r0Ⓛ vs ClusteredFib3rⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 7">
                    <h3 className="match__title">Match Day 7</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    21 Jan, 2023 to 22 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓌ vs HopertzⓁ</li>
                                        <li>somrandomguyⓁ vs GN02Ⓦ</li>
                                        <li>dinokapon26Ⓦ vs BiGFiVeWin05Ⓛ</li>
                                        <li>k_dash34Ⓛ vs musir6363Ⓦ</li>
                                        <li>BriodeeⓁ vs MABESSAⓌ</li>
                                        <li>KazremyⓌ vs FarajahchalloⓁ</li>
                                        <li>KafitiⓌ vs Andre_BeatzⓁ</li>
                                        <li>arnie_NⓁ vs alphazzz3r0Ⓦ</li>
                                        <li>raheemthefirstⓁ vs ClusteredFib3rⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 8">
                    <h3 className="match__title">Match Day 8</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    21 Jan, 2023 to 22 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>GN02Ⓓ vs onesmoⒹ</li>
                                        <li>BiGFiVeWin05Ⓦ vs HopertzⓁ</li>
                                        <li>musir6363Ⓛ vs somrandomguyⓌ</li>
                                        <li>MABESSAⓁ vs dinokapon26Ⓦ</li>
                                        <li>FarajahchalloⓁ vs k_dash34Ⓦ</li>
                                        <li>Andre_BeatzⓁ vs BriodeeⓁ</li>
                                        <li>alphazzz3r0Ⓛ vs KazremyⓌ</li>
                                        <li>ClusteredFib3rⓁ vs KafitiⓌ</li>
                                        <li>raheemthefirstⓌ vs arnie_NⓁ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 9">
                    <h3 className="match__title">Match Day 9</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    21 Jan, 2023 to 22 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓁ vs BiGFiVeWin05Ⓦ</li>
                                        <li>GN02Ⓦ vs musir6363Ⓛ</li>
                                        <li>HopertzⓁ vs MABESSAⓌ</li>
                                        <li>somrandomguyⓁ vs FarajahchalloⓁ</li>
                                        <li>dinokapon26Ⓦ vs Andre_BeatzⓁ</li>
                                        <li>k_dash34Ⓦ vs alphazzz3r0Ⓛ</li>
                                        <li>BriodeeⓁ vs ClusteredFib3rⓌ</li>
                                        <li>KazremyⓁ vs raheemthefirstⓌ</li>
                                        <li>KafitiⓌ vs arnie_NⓁ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 10">
                    <h3 className="match__title">Match Day 10</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    28 Jan, 2023 to 29 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>musir6363Ⓦ vs onesmoⓁ</li>
                                        <li>MABESSA vs BiGFiVeWin05</li>
                                        <li>FarajahchalloⒹ vs GN02Ⓓ</li>
                                        <li>Andre_BeatzⓁ vs HopertzⓌ</li>
                                        <li>alphazzz3r0Ⓦ vs somrandomguyⓁ</li>
                                        <li>ClusteredFib3rⓁ vs dinokapon26Ⓦ</li>
                                        <li>raheemthefirstⓁ vs k_dash34Ⓦ</li>
                                        <li>arnie_NⓌ vs BriodeeⓁ</li>
                                        <li>KafitiⓌ vs KazremyⓁ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 11">
                    <h3 className="match__title">Match Day 11</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    28 Jan, 2023 to 29 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓁ vs MABESSAⓌ</li>
                                        <li>musir6363Ⓦ vs FarajahchalloⓁ</li>
                                        <li>BiGFiVeWin05 vs Andre_Beatz</li>
                                        <li>GN02Ⓦ vs alphazzz3r0Ⓛ</li>
                                        <li>HopertzⓁ vs ClusteredFib3rⓌ</li>
                                        <li>somrandomguyⒹ vs raheemthefirstⒹ</li>
                                        <li>dinokapon26Ⓦ vs arnie_NⓁ</li>
                                        <li>k_dash34Ⓛ vs KafitiⓌ</li>
                                        <li>BriodeeⓁ vs KazremyⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 12">
                    <h3 className="match__title">Match Day 12</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    28 Jan, 2023 to 29 Jan, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>FarajahchalloⓁ vs onesmoⓌ</li>
                                        <li>Andre_BeatzⓌ vs MABESSAⓁ</li>
                                        <li>alphazzz3r0Ⓛ vs musir6363Ⓦ</li>
                                        <li>ClusteredFib3r vs BiGFiVeWin05</li>
                                        <li>raheemthefirstⓁ vs GN02Ⓦ</li>
                                        <li>arnie_NⓁ vs HopertzⓌ</li>
                                        <li>KafitiⓌ vs somrandomguyⓁ</li>
                                        <li>KazremyⓁ vs dinokapon26Ⓦ</li>
                                        <li>BriodeeⒹ vs k_dash34Ⓓ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 13">
                    <h3 className="match__title">Match Day 13</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    04 Feb, 2023 to 05 Feb, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓌ vs Andre_BeatzⓁ</li>
                                        <li>FarajahchalloⒹ vs alphazzz3r0Ⓓ</li>
                                        <li>MABESSAⓁ vs ClusteredFib3rⓌ</li>
                                        <li>musir6363 vs raheemthefirst</li>
                                        <li>BiGFiVeWin05 vs arnie_N</li>
                                        <li>GN02Ⓦ vs KafitiⓁ</li>
                                        <li>HopertzⓁ vs KazremyⓌ</li>
                                        <li>somrandomguyⓁ vs BriodeeⓁ</li>
                                        <li>dinokapon26Ⓛ vs k_dash34Ⓦ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 14">
                    <h3 className="match__title">Match Day 14</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    04 Feb, 2023 to 05 Feb, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>alphazzz3r0Ⓦ vs onesmoⓁ</li>
                                        <li>ClusteredFib3rⓌ vs Andre_BeatzⓁ</li>
                                        <li>raheemthefirstⓁ vs FarajahchalloⓌ</li>
                                        <li>arnie_NⓁ vs MABESSAⓌ</li>
                                        <li>Kafiti vs musir6363</li>
                                        <li>Kazremy vs BiGFiVeWin05</li>
                                        <li>BriodeeⓁ vs GN02Ⓦ</li>
                                        <li>k_dash34Ⓓ vs HopertzⒹ</li>
                                        <li>dinokapon26Ⓛ vs somrandomguyⓌ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 15">
                    <h3 className="match__title">Match Day 15</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    04 Feb, 2023 to 05 Feb, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmoⓁ vs ClusteredFib3rⓌ</li>
                                        <li>alphazzz3r0Ⓦ vs raheemthefirstⓁ</li>
                                        <li>Andre_BeatzⓌ vs arnie_NⓁ</li>
                                        <li>FarajahchalloⓁ vs KafitiⓌ</li>
                                        <li>MABESSAⓌ vs KazremyⓁ</li>
                                        <li>musir6363 vs Briodee</li>
                                        <li>BiGFiVeWin05 vs k_dash34</li>
                                        <li>GN02Ⓦ vs dinokapon26Ⓛ</li>
                                        <li>HopertzⓁ vs somrandomguyⓁ</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 16">
                    <h3 className="match__title">Match Day 16</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    11 Feb, 2023 to 12 Feb, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>raheemthefirst vs onesmo</li>
                                        <li>arnie_N vs ClusteredFib3r</li>
                                        <li>Kafiti vs alphazzz3r0</li>
                                        <li>Kazremy vs Andre_Beatz</li>
                                        <li>Briodee vs Farajahchallo</li>
                                        <li>k_dash34 vs MABESSA</li>
                                        <li>dinokapon26 vs musir6363</li>
                                        <li>somrandomguy vs BiGFiVeWin05</li>
                                        <li>Hopertz vs GN02</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div className="match__content 17">
                    <h3 className="match__title">Match Day 17</h3>

                    <div className="match__box">
                        <div className="match__group">
                            <div className="match__data">
                                <div>
                                    11 Feb, 2023 to 12 Feb, 2023
                                    <h3 className="match__name ⓌⒹⓁ">
                                        <li>onesmo vs arnie_N</li>
                                        <li>raheemthefirst vs Kafiti</li>
                                        <li>ClusteredFib3r vs Kazremy</li>
                                        <li>alphazzz3r0 vs Briodee</li>
                                        <li>Andre_Beatz vs k_dash34</li>
                                        <li>Farajahchallo vs dinokapon26</li>
                                        <li>MABESSA vs somrandomguy</li>
                                        <li>musir6363 vs Hopertz</li>
                                        <li>BiGFiVeWin05 vs GN02</li>
                                    </h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>


            </div>
        </section>
    );
};

export default Match;